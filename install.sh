...
EXTRACTED_DIR="${HOME}/tmp/xilinx-installer-${VIVADO_VERSION}"

if [[ "$MODE" == "vitis" ]]; then
    CONFIG_FILE="vitis_install_config.txt"
    SKIP_CHECK="${INSTALL_DIR}/Vitis_Embedded/${VIVADO_VERSION}/Vitis/bin/xsct"
    LIBS_SCRIPT="/opt/Xilinx/Vitis_Embedded/${VIVADO_VERSION}/Vitis/scripts/installLibs.sh"
else
...
if [[ -f "${EXTRACTED_DIR}/xsetup" ]]; then
    ...
    tar -I pigz -xvf "${TARBALL}" -C "${EXTRACTED_DIR}" --strip-components=1
fi
...
if [[ "$MODE" == "vitis" ]]; then
    docker run --rm \
        -v "${INSTALL_DIR}:/opt/Xilinx" \
        "${IMAGE_NAME}" bash -c "/opt/Xilinx/Vitis_Embedded/${VIVADO_VER
